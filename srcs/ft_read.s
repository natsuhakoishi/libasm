...
ft_read:
	mov rax, 0						; set syscall number to 0 (read), as 0 read 1 write 2 open 3 close etc.
	syscall							; CPU will switch to kernel mode, check syscall table according to syscall number and operate also return result

	; if success kernel will return byte count that read successfully, else return rax = -errno which is signed number
